...
- Konfiguracja przez FlutterFire: `lib/firebase_options.dart` oraz natywne `google-services.json` / `GoogleService-Info.plist`. **Android** w repo ma **rzeczywistą** wygenerowaną konfigurację (projekt `motosnap-18101`). **iOS i web** w `firebase_options.dart` (oraz iOS plist) mogą być **placeholderami** do czasu `flutterfire configure` na maszynie z macOS/Xcode. Brak sekretów serwerowych w repo (np. klucz Gemini tylko jako sekret Functions) — identyfikatory klienckie są zgodne z konwencją FlutterFire; dla **portfolio** publiczne trzymanie `google-services.json` jest typowe, natomiast **produkcja** powinna dodatkowo włączyć **App Check**, **restrykcje klucza API** w Google Cloud oraz opcjonalnie **osobny projekt demo** zamiast wystawiania głównego projektu w otwartym repozytorium.
