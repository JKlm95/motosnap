...
- **`lib/features/`** — `splash` (hydracja sesji), `auth` (Firebase / offline), `scan` (domena, repozytorium, sync do Firestore+Storage, UI), `history`, `map` (prywatna mapa lokalnych skanów), `settings`.
...
- **`lib/app/shell/`** — `MainShellLayout` / `kShellGlassNavContentPadding` (dolny margines treści pod pływającą nawigacją), `GlassShellBottomNav` (cztery gałęzie: 0 Skan FAB, 1 Historia, 2 Mapa, 3 Ustawienia; etykiety z `AppStrings`). Kamera embedded aktywna **tylko** przy indeksie 0 (`isScanTabActive`).
...
- [ ] **Tab switch** (Skan ↔ Historia ↔ Mapa ↔ Ustawienia) — preview wraca na Skanie, pauza na pozostałych, bez black screen.
...
- Shell: `/scan`, `/history`, `/map`, `/settings` (`StatefulShellRoute.indexedStack`) — dolny pasek: [`GlassShellBottomNav`](lib/app/shell/glass_shell_bottom_nav.dart) + [`MainShellLayout`](lib/app/shell/main_shell_layout.dart) (dolny padding treści).
...
- Konfiguracja przez FlutterFire: `lib/firebase_options.dart` oraz natywne `google-services.json` / `GoogleService-Info.plist`. W **publicznym** repozytorium trzymane są **placeholdery** (m.in. `motosnap-mvp` / klucze `AIzaSyDummy…`); prawdziwy projekt podłączasz lokalnie przez `flutterfire configure` i **nie commitujesz** produkcyjnych wartości. Plik [`.firebaserc`](.firebaserc) może nadal wskazywać domyślny projekt CLI maintainera — to identyfikator projektu, nie klucz API; dla czystego portfolio można go zmienić na placeholder. Brak sekretów serwerowych w repo (np. klucz Gemini tylko jako sekret Functions). **Produkcja:** App Check, restrykcje klucza API w Google Cloud, ewentualnie osobny projekt demo.
...
5. **Publiczny feed / community map** — pole `is_public` i `public_location_approximation` są przygotowane pod Firestore, ale **brak** publicznego query / mapy innych użytkowników. Obecna zakładka **Mapa** ([`lib/features/map/`](lib/features/map/)) to wyłącznie **prywatna mapa**: dane z lokalnego [`ScanRepository.watchScans()`](lib/features/scan/domain/scan_repository.dart), filtr GPS w [`scan_map_location_filter.dart`](lib/features/map/domain/scan_map_location_filter.dart) (odrzuca brak współrzędnych, `0,0`, poza zakresem; max 500 markerów), kafelki OpenStreetMap przez [`flutter_map`](https://pub.dev/packages/flutter_map) **bez klucza API Google Maps**. Kolejny etap public map: `isPublic == true`, osobny model/query, zaokrąglenie lokalizacji, limity, przegląd reguł bezpieczeństwa — **nie** mieszać z tą implementacją.
